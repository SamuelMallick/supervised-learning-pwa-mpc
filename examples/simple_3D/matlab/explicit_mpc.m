...
% pwa regions 1
A1 = [1, 0.5, 0.3; 0, 1, 1; 0, 0, 1];
B1 = [0; 0.1; 1];
c1 = [0; 0; 0];
sys1 = LTISystem('A', A1, 'B', B1, 'f', c1);
R1 = Polyhedron('A', [0, 1, 0; Ax], 'b', [1; bx]);
sys1.setDomain('x', R1);

A2 = [1, 0.2, 0.3; 0, 0.5, 1; 0, 0, 1];
B2 = [0; 0.1; 1];
c2 = [0.3; 0.5; 0];
sys2 = LTISystem('A', A2, 'B', B2, 'f', c2);
R2 = Polyhedron('A', [0, -1, 0; Ax], 'b', [-1; bx]);
sys2.setDomain('x', R2);
...
% cost
pwa_sys.x.penalty = OneNormFunction(diag([1, 1, 1]));

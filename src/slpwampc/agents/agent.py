...
class Agent:
    ...
    def train(
        ...
        plot_each_region : bool, optional
            If true, each region is plotted 1 by 1 with a second between
            each. By default False.
        ...
        while True:
            ...
            if plot:
                self.plot_iteration(
                    ...
                    plot_each_region=plot_each_region,
                )
    ...
    def plot_iteration(
        ...
        plot_each_region: bool = False,
        axis_lims: tuple = ((-12, 12), (-12, 12), (-12, 12)),
    ) -> None:
        ...
        plot_each_region : bool, optional
            If true, each region is plotted 1 by 1 with a second between
            each. By default False.
        axis_lims : tuple[tuple], optional
            The axis limits, each entry in the outer tuple is x, y and
            z (if three) limits. By default ((-12, 12), (-12, 12), (-12, 12))."""
        if interactive:
            self.ax.clear()

        self.ax.set_xlim(axis_lims[0][0], axis_lims[0][1])
        self.ax.set_ylim(axis_lims[1][0], axis_lims[1][1])
        if not is_2D:
            self.ax.set_zlim(axis_lims[2][0], axis_lims[2][1])
        self.ax.set_axis_off()
        ...
        # save2tikz(plt.gcf(), name=f"iter_{iter}")
        if not interactive:
            plt.show()
        else:
    ...
    # TODO saving and loading for general classifier
